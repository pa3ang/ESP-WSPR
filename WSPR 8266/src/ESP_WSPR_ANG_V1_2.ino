...
// ESP-WSPR LPF pins
#define TX_band1_LED_PIN D5   // green from pin D7 to signal TX_band1 transmittion.    40meter
#define TX_band2_LED_PIN D6   // yellow LED from pin D6 to signal TX_band3 transmittion. 20meter
#define TX_band3_LED_PIN D7   // red LED from pin D5 to signal TX_band3 transmittion.  10meter
...
void loadConfig() {
  ...
  if (strlen(config.loc) != 6 || config.dbm > 100 || config.ssid == -1 ) {
...
String htmlCalibrate() {
  ...
  html += "<style>body{font-family:verdana;text-align:center;}button{margin:5px;padding:10px;font-size:1.2em;background-color:#1fa3ec;border:0;width:150px;border-radius:.3rem;color:#fff;}</style>";
  
  html += "</head><body><h1>Si5351 Calibrate</h1>";
