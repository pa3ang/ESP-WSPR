...
Version 1.8  2026-03-17
  - inproved WSPR timing in encode() to sum up to 110.6 seconds exactly
  - cleanup webserver html generation using generic strings for <head> <style> <footer>
  - small improvements in web presentation
  - improved EEPROM integrity for clean start and Easter Egg 

Version 1.7  2025-12-22
  - made calibration routine available now without rebooting (same as band config) and possibility to cancel out.
  - improved testloop to cancel out true the webinterface and redirect to / (root).
  - improved save & reboot in config screen to refresh after 15 seconds to / (root).
  - random frequency offset -50 to +50 Hz to stay of the center frequency but well within the 200 Hz bandwidth.
  - commented out the 10MHz stabilization on clk2 as this influences WiFi connectivity.

Version 1.6
  - added more bands to FEX version 12 in total now 160 - 2 meter
...
struct Config {
    ...
    char loc[10];         // Locator
    int dbm;              // TX poser in dBm
    int cal;              // Si5351 calibration factor
    int gps;              // gps enable / disable
    int band;             // band index
    int oled;             // oled orientation
    int interval;         // TX interval
    int ssid;             // WiFi reset flag
    BandConfig bands[TOTAL_BANDS];  // 12 bands
} config;

// Connection variables
const char ver[10]        = "ANG-V1.8";
...
const char* FOOTER =
  "<p style='font-size:10px; color:grey; text-align:center'>ESP8266, Version : ANG-V1.8, Mar-2026</p></body></html>";
...
String htmlBandConfig() {
  String html = HEAD;
  html += STYLE;
  html += "<style>";
  html += "table { border-collapse: collapse; margin: auto; }";
  html += "th, td { padding: 8px 10px; text-align: center; }";
  html += "th { background-color: #1fa3ec; color: white; }";
  html += "</style>";
  html += "<body><div class='wrap'>";
  html += "<h1>ESP-WSPR Bands</h1>";
...
void handleCalibrate() {
  ...
  if (server.method() == HTTP_POST && server.hasArg("step")) {
    ...
    if (step == "save" || step == "cancel") {
      ...
      server.sendHeader("Location", "/");
      server.send(303);         // POST → GET redirect
      calibrate = false;        // cancel calibration
      cal_init = false;
...
String htmlTestLoop() {
  ...
  html += STYLE;
  html += "<body><div class='wrap'>";
  html += "<h1>ESP-WSPR TX Test</h1>";
  html += "<center><p>The OLED indicates the transmitted<br>frequency according to the<br>active band configuration.</p>";
  html += "<p>Press Reboot to resume normal operation.</p></center>";
  
  html += "<form method='POST' action='/endtestloop'>";
  html += "<button name='step' value='save'>Reboot</button>";
  html += "</form>";
  html += FOOTER;
  return html;
}
// page testloop
void handleTestloop() {
  String html = htmlTestLoop();  
  server.send(200, "text/html", html);
  tester();
}
void handleEndtestloop() {
  String html =
  "<!DOCTYPE html><html><head>"
  "<meta http-equiv='refresh' content='10;url=/' />"
  "<title>Saved</title>"
...
void setup() {
  ...
  if (!Time_set)    { 
    ...
    epochUnixNTP();  // get NTP time in one shot @ boot 
  }
  ...
  server.on("/endtestloop", HTTP_POST, handleEndtestloop);
  server.begin();
...
void encode() {
  ...
  showOLED("TX ACTIVE", String((freq+offset) / 1000000.0, 6));
  display.drawRect(10, 54, 110, 8, OLED_WHITE);

  char msg[128];
  sprintf(msg, "- TX ON - STARTING TRANSMISSION AT : %02d:%02d - %luHz %s|%s|%d LPF: %d",   hour(), minute(), (unsigned long)freq+offset, config.call, config.loc, config.dbm, selectedFilter);
  Serial.println(msg);
  ...
  unsigned long mStart = millis();  // start time
  ...
  unsigned long mEnd = millis();   // end time
  // TX off
  si5351.set_clock_pwr(SI5351_CLK0, 0);
