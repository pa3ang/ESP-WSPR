...
void loadConfig() {
  ...
  if ((strlen(config.loc) != 6) || (strcmp(config.loc, "ERASE") == 0)) {
...
String htmlCalibrate() {
  ...
  html += "<button name='step' value='save'>Save</button>";
  html += "<button name='step' value='cancel'>Cancel</button>";
  html += "</form>";
...
String htmlTestLoop() {
  String html = "<!DOCTYPE html><html><head><meta charset='UTF-8'>";
  html += "<meta name='viewport' content='width=device-width,initial-scale=1'>";
  html += "<title>ESP-WSPR Test Loop</title>";
  ...
  html += "</head><body>";
  html += "<h1>Test Loop</h1>";
  html += "<p>OLED indicates the transmitted frequency <br>according to the Band Configuration</p>";
  html += "<p>Press Reboot to go back to normal operation.</p>";
...
void setup() {
  ...
  //Serial.println("SI5351 CLK2 set to 10MHz, 2MA.");
  */
