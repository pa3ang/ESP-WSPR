...
Version 1.3
  ...
  - LPF can handle the hardware version of PA3FBX with 3 pins and LPF 1, 2 and 3 individual Connected
      ...
      LINE 216 HAS >>>>  #DEFINE REDUCED_LPF_HW   WHICH NEEDS TO BE COMMENT OUT IF THE PA3FEX PCB IS USED
...
// ESP-WSPR LPF pins     bands according PA3FBX design
...
#define EEPROM_MAGIC 0x42A7  // marker om te checken of EEPROM geldig is
...
struct Config {
    ...
    int interval;
    int ssid;
    BandConfig bands[TOTAL_BANDS];  // 8 bands
} config;

// Common variables
const char ver[10]        = "ANG-V1.3";
...
// THIS IS THE DESIGNATOR BETWEEN PA3FBX DESIGN AND THE PA#FEX PCB WITH MORE LPF
// COMMENT LINE OUT IF USING THE PA3FEX PCB WITH 7 LPF POSSIBILITY
#define REDUCED_LPF_HW

// 
uint8_t mapLPF(uint8_t logicalLPF) {
#ifdef REDUCED_LPF_HW
    switch (logicalLPF) {
...
WiFiManager wm;
WiFiManagerParameter custom_gps("gps", "OFF GRID ? GPS (0 of 1)", "0", 2); // id, label, default, lengte
...
String htmlForm() {
  ...
  html += "Interval (0,2,4,6 ...) minutes<br><input name='interval' value='" + String(config.interval) + "'><br>";
...
void handleSave() {
  ...
  if (config.interval < 0 || config.interval > 16 || (config.interval % 2 != 0)) { config.interval = 4; }
...
String htmlBandConfigPage() {
  ...
  for (int i = 0; i < TOTAL_BANDS; i++) {
    ...
    #else
    const int maxLPF = 8;   // full hardware
    #endif
...
String htmlCalibrate() {
  ...
  html += "<button name='step' value='o'>+1 kHz</button>";
  html += "<button name='step' value='l'>-1 kHz</button><br>";
  html += "<button name='step' value='p'>+10 kHz</button>";
  html += "<button name='step' value=';'>-10 kHz</button><br><br>";
...
void handleCalibrate() {
  ...
  if (server.method() == HTTP_POST && server.hasArg("step")) {
    ...
    int32_t stepVal = 0;
    switch (c) {
      case 'y': stepVal = 1; break;
      case 'h': stepVal = -1; break;
      case 'u': stepVal = 10; break;
      case 'j': stepVal = -10; break;
...
void loop() {
  ...
  if (!calibrate) {  // alleen als niet in calibratie
    ...
    // find next band to transmit
    int activeCount = 0;
    // Count active bands
    for (int i = 0; i < TOTAL_BANDS; i++) {
      if (config.bands[i].active) activeCount++;
    }
    ...
    // Wrap if bandStep exceeds activeCount
    if (bandStep >= activeCount) bandStep = 0;

    // Find the next active band
    int stepCounter = 0; 
    ...
    for (int i = 0; i < TOTAL_BANDS; i++) {
      ...
      // If somehow nothing found, bail out
      if (!nextBandName) {
        Serial.println("⚠️ No matching active band found!");
      }
      ...
      // Display the selected band info
      //Serial.printf("Next Band: %s | Freq: %u | Filter: %d\n",
      //           nextBandName, freq, selectedFilter);
    ...
    if (s == 0 && m == startMinute) {
      ...
      // calculate next slot .. user is responsible for timing
    ...
    // OLED update 
    showOLED("IDLE " + String(countdownSec) + " s", String(h) + ":" + (m<10?"0":"") + String(m) + ":" + (s<10?"0":"") + String(s), String("Nxt:") + nextBandName + String(" F") + String(selectedFilter)
    );
  }
}
